"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WasteService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const waste_entity_1 = require("./entities/waste.entity");
const typeorm_2 = require("typeorm");
let WasteService = class WasteService {
    constructor(wasteRepository) {
        this.wasteRepository = wasteRepository;
    }
    async create(createWasteDto) {
        try {
            const waste = this.wasteRepository.create(createWasteDto);
            await this.wasteRepository.save(waste);
            return { waste };
        }
        catch (e) {
            console.log('ERROR TO CREATE WASTE');
            console.log(e.detail);
            throw new common_1.BadRequestException(e.detail);
        }
    }
    async findAll() {
        const wastes = await this.wasteRepository.find();
        return { wastes };
    }
    async findOne(id) {
        const waste = await this.wasteRepository.findOneBy({ id });
        if (!waste)
            throw new common_1.NotFoundException(`Waste not found`);
        return { waste };
    }
    async update(id, updateWasteDto) {
        const { waste } = await this.findOne(id);
        const params = Object.assign(Object.assign(Object.assign({}, waste), updateWasteDto), { updatedAt: new Date() });
        try {
            await this.wasteRepository.save(params);
            return { message: `Wasted updated successfully` };
        }
        catch (e) {
            console.log('ERROR TO UPDATED WASTE');
            console.log(e);
            throw new common_1.BadRequestException(e.detail);
        }
    }
    async remove(id) {
        await this.findOne(id);
        await this.wasteRepository.delete({ id });
        return { message: 'Waste deleted successfully' };
    }
};
WasteService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(waste_entity_1.Waste)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], WasteService);
exports.WasteService = WasteService;
//# sourceMappingURL=waste.service.js.map